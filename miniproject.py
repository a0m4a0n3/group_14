...
def main():
    ...
    if enter_button:
        ...
        if selected_city!=" " and postal_code!=" " and selected_sub_category!=" " and product_name!=" " and year and month:
            ...
            if lat is not None and lon is not None:
                ...
                # Insert user input into MySQL database
                try:
                    # SQL query to insert user input
                    sql = "INSERT INTO input (City, `Postal Code`, `Sub-Category`, `Product Name`, Quantity, Discount, Year, Month, `Predicted sales`) VALUES (%s, %s, %s, %s, %s, %s, %s, %s, %s)"

                    
                    # Tuple containing user input values
                    values = (selected_city, postal_code, selected_sub_category, product_name, quantity, discount, year, month, str(predicted_sales))
                ...
                predicted_sales_str = str(predicted_sales)
                ...
                \u2022 Year: {year}
                \u2022 Month: {month}
                \u2022 predicted sales: {predicted_sales_str}
                """
                tooltip_text = f"""City: {selected_city}, Year: {year}, Month: {month}, Sales: {predicted_sales_str}"""
                folium.Marker(location, tooltip=tooltip_text, permanent=True).add_to(m)
    ...
    # Close cursor and connection when the app is finished executing
    cursor.close()
    conn.close()
